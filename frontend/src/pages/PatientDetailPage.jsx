...
export default function PatientDetailPage({ token }) {
  ...
  const loadPatientContext = useCallback(async () => {
    ...
    setInlineMessage('')
    try {
      const [nextPatient, nextPredictions, nextExplanations, nextObservations, nextNotes] = await Promise.all([
            ...
            {observations.map((obs) => (
              <li key={obs.id} className="rounded border border-slate-200 p-3">
                ...
                <p className="text-slate-600">
                  HR {obs.heart_rate ?? '—'} • SBP {obs.systolic_bp ?? '—'} • DBP {obs.diastolic_bp ?? '—'} • SpO₂{' '}
                  {obs.oxygen_saturation ?? '—'} • Glucose {obs.glucose ?? '—'}
                </p>
              </li>
            ))}
